 ...
 */ 
 ...
 .org $0C
 rjmp isr_tcl_tflag					;jump to isr
...
.list
 ...
 .DSEG
 audio_toggle:		.byte 1	;save 1 byte for audio on=$FF off=$00
 hold_enable:		.byte 1 ;save 1 byte for hold/run mode entered
 hold_val:			.byte 1 ;save 1 byte for hold/run value
...
RESET:
	...
	out DDRC, r16				;Port-C (Backlight & Buzzer)
	sbi PORTC, 0				;turn buzzer off
	cbi PORTC, 1				;turn on backlight
	ldi r16, $F7
	out DDRA, r16				;PORT-A (Audio), except PA3(BUSY)
	out PORTA, r16				;initial values for audio device
	...
	ldi r16, $FF
	sts audio_toggle, r16		;audio is on
main:
		
	ldi r16, $00
	sts hold_enable, r16		;hold/run is not enetered
	sts hold_val, r16			;hold/run is set to run
	sts cap_store_val, r16		;disable cap/store
	sts see_stored_val, r16		;preset to 0
...
//start 5-sec sleep counter
	...
	rjmp freq_once				;enter capture screen
	sbis PIND, 6				;skip if RT not pressed
	...
	rjmp run_once				;enter run screen
	sbis PIND, 6				;skip if RT not pressed
	rjmp freq_once				;enter hold screen
	;********************************************
	brtc main_jump				;return to main through jmp
	;********************************************
	...
	rjmp hold_once				;enter hold screen
	;************************************************
	brtc main_jump				;unable to reach main on its own
	;OUT-OF-REACH ERROR			;jump to main_jump to then make
								;the jump from there
	;************************************************
...
;*******************************************************************
;audio-self contained subroutine that toggles on/off the audio for 
;the capture mode
;access SRAM audio_toggle
;$FF=audio on
;$00=audio off
...
audio:
	...
	andi r16, $F8				;keep PBSW
	cpi r16, $F8				;compare to all PBSWs high
	brne audio					;wait for all to be high
	lds r16, audio_toggle		;read current audio preference
	com r16
	sts audio_toggle, r16		;set new audio preference
	rcall tone_5v				;alert change in preference
	ret
...
capture_mode:
	cbi PORTC, 1			;turn on backlight
	ldi r16, $00			;load all 0's to...
	out TCCR1B, r16			;stop TC1
...
//check audio preference
	lds r16, audio_toggle		;check audio preference
	cpi r16, $00				;check if off
	breq checkpoint				;if yes, skip audio
...
overrange:
	rcall clr_dsp_buffs
	ldi ZL, LOW(overrange_msg<<1)
	ldi ZH, High(overrange_msg<<1)
	rcall load_msg
...
hold_mode:
	...
	clt
	ldi r16, $FF
	sts hold_enable, r16			;we are in hold/run mode
...
display_run_h:
	...
	brts end_run_h
//user want to hold???
	lds r16, hold_val			;$FF=yes, $00=no
	cpi r16, $FF				;checking....
	breq holding				;
//otherwise, no.
	rjmp running_h				;repeat
	end_run_h:
	clt
	ret
holding:
	ldi ZH, High(line12_message<<1)
	ldi ZL, Low(line12_message<<1)
	rcall load_msg
	rcall update_lcd_dog		;display buffers onto lcd
	holding_loop:
	lds r16, hold_val			;check hold val
	cpi r16, $00				;if it is off...
	breq running_h				;run again
	brts end_run_h			;end when user wants to
	rjmp holding_loop				;wait for user input
...
store_mode:
	...
	ldi r16, $00
	sts old_voltage_val, r16
	sts old_voltage_val+1, r16
	sts old_voltage_val+2, r16
	sts old_voltage_val+3, r16
	sts old_voltage_val+4, r16
	sts old_voltage_val+5, r16
	sts old_voltage_val+6, r16
	sts old_voltage_val+7, r16
	sts old_voltage_val+8, r16
	sts old_voltage_val+9, r16
	sts old_voltage_val+10, r16
	sts old_voltage_val+11, r16
	sts old_voltage_val+12, r16
	sts old_voltage_val+13, r16
	sts old_voltage_val+14, r16
	sts old_voltage_val+15, r16
	rcall default_buff			;set up buffer for display
	rcall update_lcd_dog		;display buffers onto lcd
	rcall delay_10mS			;prevent switch bouncing
	in r16, PIND				;read PBSW
...
toggle_isr:
	...
	brne bounce			;no, bounce occurred

	//test hold
	
	lds r16, hold_enable	;see if we are coming from...
	cpi r16, $FF			;hold/run mode.
	brne no_hold
	com r18					;make 1's=0's, vice versa
	andi r18, $80			;isolate PD7(GO)
	cpi r18, $80			;check is GO was pressed
	
	breq toggle_hold			;it was, toggle hold val
						;otherwise, carry on with t-flag
...
toggle_hold:
	lds r16, hold_val	;check what it is now
	com r16				;complement it
	sts hold_val, r16	;send out toggled value
	pop r16
	out SREG, r16
	rjmp end_isr
...
;start_tc1- is a self-contained subroutine in which all setup necessary
;for the tc1 will be initialized.
;
;Clock set to be divided by 64 prescaler
;Compare with A output
;
;registers used: r16
;				 r17
;inputs:
;outputs: TCCR1B (Timer counter 1 control register)
;		  TIMSK (Timer counter masking register)
;

start_tc1:
		ldi     r16,0x04        ;(CLK/256 src)
        out     TCCR1B,r16
		ldi		r16, $00		;load 0's...
		out		TCNT1H, r16		;to clear the counter
		out		TCNT1L, r16		;"					"
		ldi     r17, $4C		;load high byte
		ldi		r16, $4B		;load low byte
		out		OCR1AH, r17		;output high byte
		out		OCR1AL, r16		;output low byte
        ldi     r16,0x10        ;enable Timer1 Output Compare A
        out     TIMSK,r16
		ret

;isr_tcl_tflag-self-contained interrupt subroutine.
;Can only be reached when the interrupt flag for the 
;TC1's compare register is activated.
;
;Turn off backlight
;stop TC1 
;
...
isr_tcl_tflag:
		push r16				;save the r16 value coming in
		in r16, SREG			;load SREG...
		push r16				;to save it
		sbi PORTC, 1			;turn off backlight
		ldi r16, $00			;load all 0's to...
		out TCCR1B, r16			;stop TC1
		pop r16					;return SREG
		out SREG, r16			;"			"
		pop r16					;return r16
		reti					;return w/ I-Flag
...
line9_message: .db 3, "HOLD/RUN        ", 0	
line10_message: .db 3, "Cap/Store Mode? ", 0
line11_message: .db 3, "    Voltage     ", 0
line12_message: .db 3, "HELD            ", 0
overrange_msg:  .db 2, " OVER RANGE     ", 0
freq_message:   .db 3, "  Freq Mode?    ", 0
